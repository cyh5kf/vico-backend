...
export default class RealtimeSuccessChart extends React.Component {
    ...
    getFilterOption = () => {
        ...
        for (let key of dailyStatusData) {
            var { tstamp, accept, failed } = key;
            var tstampFormat = moment(tstamp).format("YYYY-MM-DD");
            if (accept !== 0) {
                const successRate = ((1 - failed / accept) * 100).toFixed(1);
                x_time_data.push(tstampFormat);
                y_daily_data.push(successRate);
            }
        }
    ...
    getAllOption = () => {
        ...
        for (let key of dailyInternalStatusData) {
            ...
            if (accept !== 0) {
                ...
                y_internal_data.push(successRate);
            }
        }
        ...
        for (let key of dailyTransnationalStatusData) {
            ...
            if (accept !== 0) {
                ...
                y_transnational_data.push(successRate);
            }
        }
    ...
    render() {
        ...
        return (
            <Card>
                <Echart refName="dailySuccessChart" options={setOption} style={{ height: 400 }}></Echart>
            </Card>
        );
    }
}
